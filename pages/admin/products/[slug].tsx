...
const ProductAdminPage: FC<Props> = ({ product }) => {
    ...
    const onNewTag = (key: KeyboardEvent<HTMLDivElement>) => {
        const tagsValues = getValues("tags")
        if (key.code === "Space" && newTagValue.length > 0) {
            setNewTagValue('')
    ...
    const onFilesSelected = async ({ target }: ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            console.log("🚀 ~ file: [slug].tsx ~ line 139 ~ onFilesSelected ~ error", error)

        }
    }

    const onDeleteImage = (image: string) => {
        setValue("images", getValues("images").filter(i => i !== image), {
            shouldValidate: true
        })
    }
